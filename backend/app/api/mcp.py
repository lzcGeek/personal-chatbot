...
async def add_server(
    ...
    server = McpServer(
        ...
        env=payload.env,
        requires_network=payload.resolved_requires_network,
    )
    try:
...
async def enable_server(
    ...
    if not await request.app.state.mcp_manager.set_enabled(server_id, user.id, True):
...
async def disable_server(
    ...
    if not await request.app.state.mcp_manager.set_enabled(server_id, user.id, False):
        raise HTTPException(status_code=404, detail="MCP server not found")
...
async def update_server(
    ...
    async with SessionLocal() as session:
        ...
        if server is None:
            raise HTTPException(status_code=404, detail="MCP server not found")
        server.requires_network = payload.requires_network
        await session.commit()
        await session.refresh(server)
        request.app.state.mcp_manager.update_network_policy(
            server.id, server.requires_network
        )
