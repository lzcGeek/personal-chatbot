...
The system SHALL periodically extract key facts and summaries from conversations and store them as vector embeddings for long-term memory retrieval.

#### Scenario: Fact extraction after conversation
- **WHEN** a conversation exchange completes
- **THEN** the system asynchronously extracts key facts and stores them as vector embeddings in ChromaDB

#### Scenario: Memory retrieval on new message
- **WHEN** the user sends a new message
- **THEN** the system queries ChromaDB for semantically relevant memories and includes them in the LLM context
...
- **WHEN** retrieved memories have similarity scores above the threshold
